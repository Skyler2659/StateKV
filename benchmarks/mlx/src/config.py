...
class EvictionConfig:
    ...
    # Paper-method knobs
    adakv_safeguard_alpha: float = 0.2
    curdkv_projection_dim: int = 20
    curdkv_num_sink: int = 4
    keydiff_recent_ratio: float = 0.0
    # Experimental conditional / query-conditioned geometry methods.  Ridge
    # coefficients are relative to the mean diagonal of the fitted Gram
    # matrix, which keeps the same config usable across layers and heads.
    conditional_ridge_lambda: float = 1e-3
    conditional_leverage_lambda: float = 1e-3
    attention_residual_ridge_lambda: float = 1e-3
    attention_residual_budget_ratio: float = 0.5
    attention_weighted_ridge_lambda: float = 1e-3
    attention_weight_epsilon: float = 1e-4
    joint_kv_gamma: float = 0.5
    joint_kv_ridge_lambda: float = 1e-6
    ridge_budget_lambda_floor: float = 1e-4
    diversity_ridge_lambda: float = 1e-3
    diversity_candidate_multiplier: int = 4
    # Compactor faithful prefill compression knobs
    compactor_sketch_dim: int = 48
    compactor_chunk_size: int = 0  # 0 = full-context leverage fit (paper default)
...
class BenchmarkConfig:
    ...
    sample_strategy: str = "first"  # first, random, explicit
