"""Legacy text deletion diagnostic (not a cache-state intervention).
...
class CounterfactualDeletion:
    """Legacy retokenizing ablation, disabled unless explicitly requested.
    ...
    def run(
        ...
        if not allow_text_retokenization:
            raise RuntimeError(
                "CounterfactualDeletion removes input tokens and recomputes all "
                "positions/states; it is not a fixed-snapshot KV intervention. "
                "Pass allow_text_retokenization=True only for an explicitly "
                "labelled legacy text-ablation diagnostic."
            )
