"""Legacy text-subsequence restoration (not cache-state restoration).
...
class RestorationExperiment:
    """Legacy retokenizing restoration, disabled unless explicitly requested.
    ...
    def run(
        ...
        device: str = "cuda",
        allow_text_retokenization: bool = False,
    ) -> RestorationResult:
        ...
        """
        if not allow_text_retokenization:
            raise RuntimeError(
                "RestorationExperiment rebuilds a shortened input sequence and "
                "therefore changes RoPE positions and contextual states. It is "
                "not fixed-snapshot cache restoration. Pass "
                "allow_text_retokenization=True only for a labelled legacy diagnostic."
            )
