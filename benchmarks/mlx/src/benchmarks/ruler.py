...
class RULERBenchmark(BaseBenchmark):
    ...
    def __init__(
        ...
        self.sample_strategy = str(sample_strategy or "first").lower()
    ...
    def _load_official_rows(self, task_name: str) -> List[Dict[str, Any]]:
        ...
        total = len(ds)
        count = min(total, int(self.n_samples_per_task or total))
        if self.official_sample_indices is not None:
            indices = list(self.official_sample_indices)
            invalid = [index for index in indices if index < 0 or index >= total]
            if invalid:
                raise ValueError(f"official sample indices out of range: {invalid[:10]}")
            indices = indices[:count]
        elif self.sample_strategy == "random":
            task_seed = sum((i + 1) * ord(ch) for i, ch in enumerate(hf_task))
            indices = sorted(random.Random(self.seed + task_seed).sample(range(total), count))
        elif self.sample_strategy == "first":
            indices = list(range(count))
        else:
            raise ValueError(
                f"unsupported RULER sample_strategy={self.sample_strategy!r}; "
                "use first, random, or official_sample_indices"
            )
        rows = []
        for index in indices:
            row = dict(ds[int(index)])
            row["_official_dataset_index"] = int(index)
            rows.append(row)
        return rows
    ...
    def _official_row_to_sample(self, tokenizer, row: Dict[str, Any], task_name: str) -> Dict[str, Any]:
        ...
        if str(official_task).lower() in {"vt", "variable_tracking"}:
            evidence = parse_vt_prompt(prompt, tokenizer)
        elif str(official_task).lower().startswith("niah") and answers:
            answer = str(answers[0])
            answer_at = context.find(answer)
            if answer_at >= 0:
                char_start = max(0, context.rfind(".", 0, answer_at) + 1)
                char_end = context.find(".", answer_at + len(answer))
                char_end = len(context) if char_end < 0 else char_end + 1
                _, _, positions = _token_span_from_char_span(
                    tokenizer, prompt, char_start, char_end
                )
                evidence = {
                    "parser_version": "ruler_niah_sentence_v1",
        ...
        return {
            ...
            "full_text": text,
            "answer_text": answer_text,
            "evidence_positions": evidence.get("evidence_positions", []),
            "path_annotation": evidence if str(official_task).lower() in {"vt", "variable_tracking"} else None,
            "metadata": {
                "task": official_task,
                ...
                "official_dataset_index": row.get("_official_dataset_index"),
                "sample_strategy": (
                    "explicit" if self.official_sample_indices is not None else self.sample_strategy
                ),
                **{
                    key: value
                    for key, value in evidence.items()
                    if key not in {"evidence_positions", "assignments"}
                },
            },
        }
    ...
    def prepare_samples(self, tokenizer) -> List[Dict[str, Any]]:
        ...
        for task_name in self.tasks:
            ...
            for si in range(self.n_samples_per_task):
                ...
                evidence: Dict[str, Any] = {}
                if task_name in {"variable_tracking", "vt"}:
                    evidence = parse_vt_prompt(
                        prompt, tokenizer, target_value=str(answer)
                    )
                elif task_name in {"niah_single", "niah_single_1"} and raw.get("needle"):
                    char_start = prompt.find(str(raw["needle"]))
                    if char_start >= 0:
                        char_end = char_start + len(str(raw["needle"]))
                        _, _, positions = _token_span_from_char_span(
                            tokenizer, prompt, char_start, char_end
                        )
                        evidence = {
                            "parser_version": "synthetic_ruler_niah_v1",
                            "evidence_positions": positions,
                            "evidence_char_start": char_start,
                            "evidence_char_end": char_end,
                        }
                ...
                samples.append({
                    ...
                    "answer_text": answer_text,
                    "evidence_positions": evidence.get("evidence_positions", []),
                    "path_annotation": evidence if task_name in {"variable_tracking", "vt"} else None,
                    "metadata": {
                        "task": task_name,
                        "answer": answer,
                        "answers": raw.get("answers", [answer]),
