...
class L1LeverageEviction(BaseEviction):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.score_source = {"value": "v", "key": "k", "key_value_concat": "kv"}.get(
            self.score_source, self.score_source
        )
        if self.score_source not in {"v", "k", "kv"}:
            raise ValueError(f"unsupported L1 score_source={score_source!r}")
        ...
        self.estimator_diagnostics: dict[int, dict] = {}
    ...
    def compute_scores(self, layer_k, layer_v, layer_idx, **kw):
        ...
        scores = est.scores(rows, force_refit=force_refit)
        self.estimator_diagnostics[int(layer_idx)] = dict(est.last_diagnostics)
        return scores
