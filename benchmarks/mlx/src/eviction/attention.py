...
class AttentionEviction(BaseEviction):
    ...
    def compute_scores(self, layer_k, layer_v, layer_idx, **kw):
        ...
        if scores is not None and scores.numel() >= seq_len:
            return scores[:seq_len]
        raise RuntimeError(
            f"attention scores unavailable for layer={layer_idx}, seq_len={seq_len}; "
            "refusing silent recency fallback"
        )
    ...
    def select_indices(self, scores, seq_len, budget, device):
        ...
        if scores is None:
            raise RuntimeError("attention selection received no attention scores")
    ...
    def on_cache_pruned(self, layer_idx, keep, previous_seq_len):
        scores = self._acc_scores.get(layer_idx)
        if scores is not None and scores.numel() >= previous_seq_len:
            self._acc_scores[layer_idx] = scores[:previous_seq_len][
                keep.to(scores.device)
            ]
...
class WindowedAttentionEviction(AttentionEviction):
    ...
    def on_cache_pruned(self, layer_idx, keep, previous_seq_len):
        super().on_cache_pruned(layer_idx, keep, previous_seq_len)
        buf = self._windows.get(layer_idx, [])
        self._windows[layer_idx] = [
            values[:previous_seq_len][keep.to(values.device)]
            for values in buf
            if values.numel() >= previous_seq_len
        ]
