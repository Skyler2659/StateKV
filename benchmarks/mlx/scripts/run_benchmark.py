...
def parse_args():
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="Refresh leverage/geometry scores every N real eviction decisions",
    )
    p.add_argument(
        "--update_policy",
        type=str,
        default=None,
        choices=["every_n_steps", "prefill_only", "never_after_prefill", "decode_only"],
    )
    ...
    p.add_argument(
        "--allow_paper_run",
        action="store_true",
        help="Explicitly acknowledge research gates before running configs/experiments/paper",
    )
    return p.parse_args()
...
def main():
    ...
    if args.config:
        ...
        paper_root = (_PROJECT_ROOT / "configs" / "experiments" / "paper").resolve()
        if paper_root in config_path.parents and not args.allow_paper_run:
            raise SystemExit(
                "Paper configs are execution-gated. Complete/review the Phase-1 decision "
                "gate, then rerun with --allow_paper_run. Use configs/experiments/dev or "
                "diagnostics for smoke and implementation checks."
            )
    ...
    if args.num_samples:
        ...
        # A CLI request for N total samples must not be silently capped by a
        # smaller per-task loader limit from the YAML.  Multi-task benchmarks
        # are still truncated to ``num_samples`` after preparation.
        cfg.benchmark.n_samples_per_task = max(
            int(cfg.benchmark.n_samples_per_task), int(args.num_samples)
        )
    ...
    if args.update_interval is not None:
        cfg.eviction.update_interval = args.update_interval
    if args.update_policy is not None:
        cfg.eviction.update_policy = args.update_policy
    if args.max_new_tokens is not None:
        cfg.benchmark.max_new_tokens = args.max_new_tokens
    ...
    # Persist CLI-resolved sweep values in config.yaml/run_manifest rather than
    # leaving the source YAML's default method or budget in provenance.
    cfg.methods = list(methods)
    cfg.cache_budgets = list(budgets)
    ...
    bundle.save_run_metadata(out_dir, model_info, samples=samples)
        ...
        for method_name in methods:
            for sample_idx, sample in enumerate(samples):
                ...
                result.update({
                    "experiment_name": cfg.experiment_name,
                    "run_id": cfg.run_id,
                    "code_commit_hash": bundle.run_provenance.get("git_commit"),
                    "repository_state_id": bundle.run_provenance.get("repository_state_id"),
                    "config_hash": bundle.run_provenance.get("config_hash"),
                    "sample_manifest_hash": bundle.run_provenance.get("sample_manifest_hash"),
                    "sample_id": sample_idx,
                    "method": method_name,
                    ...
                    "mechanism_artifacts_stale_reason": (
                        "legacy layer-only scores/selections are not phase-locked to one snapshot"
                    ),
                })
                ...
                if eviction is not None and hasattr(eviction, "last_scores"):
                    ...
                    if stats:
                        ...
                        if method_needs_attentions(method_name):
                            log_stats = stats.get("raw_score_stats", stats)
                            log_top = log_stats.get(
                                "top_values", stats.get("top_score_values", [])
                            )
                            logger.info(
                                ...
                                method_name,
                                sample_idx,
                                log_stats.get("min", float("nan")),
                                log_stats.get("max", float("nan")),
                                log_stats.get("mean", float("nan")),
                                log_stats.get("std", float("nan")),
                                log_top,
                            )
