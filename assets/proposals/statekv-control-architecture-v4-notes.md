# StateKV control architecture v4 — visual contract

- Artifact: layout-fixed revision of `statekv-control-architecture-v3.svg`; same panel map, module inventory, and evidence content, with text-overflow, overlap, and caption-placement defects repaired.
...
- Rendering note: the PNG is produced through a zero-margin HTML wrapper (`overflow:hidden`) in headless Chrome with an oversized window, then cropped to 3072×1920; screenshotting the bare SVG clips the bottom ~50 units because of window chrome/scrollbars.
...
| B3 lane tag overlapped the lane title | v4 SVG/PNG | High | Moved the "training-free candidate · P32" tag right of the measured title extent (x 238→272) | Resolved |
| Oracle lane tag text overflowed its pill | v4 SVG/PNG | Medium | Widened the "reference controller · P25–P31" pill from 170 to 236 units | Resolved |
| Evaluation-reference divider label overflowed its pill | v4 SVG/PNG | Medium | Widened the pill from 296 to 344 units and centered it on the divider line | Resolved |
| "one state · two control routes" overflowed its pill | v4 SVG/PNG | Low | Widened the pill from 230 to 250 units and centered the label | Resolved |
| Adaptive KV stack caption overlapped the L35 bar and ran past the panel | v4 SVG/PNG | High | Raised the panel to 216 units, moved the caption below the last bar row, and shortened the label to "fixed global budget · varying layers" | Resolved |
| Cold KV backing title and both description lines overflowed the dashed panel | v4 SVG/PNG | High | Restructured the panel: icon plus title on one row, two full-width tiny lines below, panel moved up to keep outer padding | Resolved |
| Vertical "recover" label overlapped the decode box | v4 SVG/PNG | Medium | Replaced the rotated label with a horizontal one in the gap between the decode box and the cold-backing panel | Resolved |
| "Layer-wise cache action" title wrapped 16/6 characters | v4 SVG/PNG | Low | Rebalanced the break to "Layer-wise" / "cache action" | Resolved |
| "exact KL / local risk" sat flush against the module edge | v4 SVG/PNG | Low | Shifted the pair left and changed the separator to a middle dot | Resolved |
| P32 card body line exceeded the card width | v4 SVG/PNG | Medium | Shortened to "fixed global allocation; best task-score point estimate" | Resolved |
| Layer-difficulty caption touched the L27 bar | v4 SVG/PNG | Low | Moved the bar rows up 7 units and the caption down 5 units | Resolved |
| SVG `<title>` disagreed with the hero heading | v4 SVG | Low | Aligned the accessible title with the hero text | Resolved |
| Headless-Chrome screenshot clipped the bottom ~50 units (window chrome/scrollbar) | v4 PNG | Medium | Rendered through a zero-margin HTML wrapper with `overflow:hidden` and an oversized window, then cropped to 3072×1920 | Resolved |
