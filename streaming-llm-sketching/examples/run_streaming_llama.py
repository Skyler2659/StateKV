...
@torch.no_grad()
def greedy_generate(model, tokenizer, input_ids, past_key_values, max_gen_len, kv_cache):
    outputs = model(
        input_ids=input_ids,
...
def main(args):
    ...
    effective_strategy = args.kv_strategy
    if args.enable_streaming and args.kv_strategy == "plain":
        # Backward compatibility for old CLI behavior.
        effective_strategy = "streaming"

    if effective_strategy == "plain":
        kv_cache = enable_plain_llm(model)
    elif effective_strategy == "streaming":
        kv_cache = enable_streaming_llm(
            model, start_size=args.start_size, recent_size=args.recent_size
        )
    elif effective_strategy == "l1_robust":
